...
| 03 | `content-engine:03-research` — research layer: Fathom-дзвінки, власний потік, LinkedIn, коменти, TikTok, Reddit → Idea Pool | щотижня (cron) | ✅ |
...
| 07 | `content-engine:07-repurpose` — переможці (топ за рік / Posts DB) → переробка в нові формати → Idea Pool | бекфіл раз + щомісяця | ✅ |
| 08 | `content-engine:08-engage` — комент-радар: `targets` (4 аудиторії → boolean → список профілів) · `radar` (свіжі пости → драфти коментарів) · `log` (факт + майнінг у research) | targets раз + добір; radar 1–2×/день | ✅ |
| 09 | `content-engine:09-track` — трекінг: інжест експортів → архів + Posts DB, скоринг (резонанс × діалоги), місячне калібрування ваг у стратегію | інжест+скор щотижня; калібрування щомісяця | ✅ |
...
> **Трекінг замикає цикл:** `09-track` пише метрики в Posts DB, ставить тіри й раз на місяць
> кладе відкалібровані ваги в Секцію 7 стратегії — звідки їх бере `04-weekly-plan`. Без нього
> рекомендатор скорить на дефолтних гіпотезах, а `07-repurpose --from-tracking` не має входу.
...
2. Кроку нема в аргументах → подивись стан клієнта в Notion і запропонуй наступний логічний:
   ...
   - пости опубліковані, метрики не інжестнуті → `content-engine:09-track`.
