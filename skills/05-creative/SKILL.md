...
- **infographic** → виклич `infographic`. Перед вибором форми скіл читає свій каталог
  `references/infographic-templates.md`: 24 форми, до кожної написано, який зміст вона несе і
  як її зазвичай псують. Там же **лог ротації**: форма не повторюється двічі поспіль і не
  частіше двох разів на п'ять креативів. Дефолт лишається chart > text-grid, коли в даних є
  форма (частка, спад по етапах, зміна в часі); коралова підсвітка = білий текст; фон лише білий.
